...
int main(int argc, char **argv) {
	...
	cv::GaussianBlur(mix_hue_image, mix_hue_image, cv::Size(9, 9), 2, 2);

	// Use the Hough transform to detect circles in the combined threshold image
	std::vector<cv::Vec3f> circles;
	cv::HoughCircles(mix_hue_image, circles, CV_HOUGH_GRADIENT, 1, mix_hue_image.rows/8, 100, 20, 0, 0);
	...
	cv::namedWindow("Threshold blue hue images", cv::WINDOW_AUTOSIZE);
	cv::imshow("Threshold blue hue images", blue_hue_image);
	cv::namedWindow("Combined red hue threshold images", cv::WINDOW_AUTOSIZE);
	cv::imshow("Combined red hue threshold images", red_hue_image);
	cv::namedWindow("Combined mix hue threshold images", cv::WINDOW_AUTOSIZE);
	cv::imshow("Combined mix hue threshold images", mix_hue_image);
